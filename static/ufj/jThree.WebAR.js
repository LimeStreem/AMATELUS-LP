...
jThree.update( function( d, e ) {
	...
	dataArr.forEach( function( data ) {
		...
		data.canvas.changed = !data.beforeUpdate || data.beforeUpdate() !== false;

		if ( delta === 0 && data.canvas.changed ) {

			var w = data.glCanvas.width;
			var h = data.glCanvas.height;
			var isPortrait = w < h;
			var scale = 0;
			...
			if ( isPortrait ) {
				...
				data.ctx.setTransform(Math.cos(rad), Math.sin(rad), -Math.sin(rad), Math.cos(rad), W/2, H/2);
				//tranformで移動した分だけtranslateで戻して原点を0，0に
				data.ctx.translate( -1 * W/2, -1 * H/2 ); 
			}

			data.ctx.drawImage( data.glCanvas, ( W - w * scale ) * .5, ( H - h * scale ) * .5, w * scale, h * scale );

			if ( isPortrait ) data.ctx.restore();

			var detected = data.detector.detectMarkerLite(data.raster, 100);

			for (var idx = 0; idx<detected; idx++) {
				var id = data.detector.getIdMarkerData(idx);
				var currId;
				...
				if (!data.pastResults[currId]) {
					data.pastResults[currId] = {};
				}

				// get the transform matrix for the marker
				// getTransformMatrix copies it to resultMat
				data.detector.getTransformMatrix(idx, data.resultMat);

				data.pastResults[currId].age = 0;
				data.pastResults[currId].transform = Object.asCopy(data.resultMat);
			}

			for (var i in data.pastResults) {
				var r = data.pastResults[i];
				if (r.age > 5) delete data.pastResults[i];
				r.age++;
			}

			for (var i in data.pastResults) {
				var mat = data.pastResults[i].transform;
				...
				matrix.decompose( frame.position, q, frame.scale );
				var euler = new THREE.Euler;
				euler.setFromQuaternion( q, "YXZ" );
				euler.x = -euler.x;
				euler.y = -euler.y;

				frame.quaternion.setFromEuler( euler, "YXZ" );
				frame.position.z = -frame.position.z;
				frame.parent.rotate.z = isPortrait ? Math.PI * .5 : 0;

				frame._webarPlaying = true;
				clearTimeout( frame._webarClearTimeout );
				frame._webarClearTimeout = setTimeout( function() {
					var i = this[ 0 ];
					data.frames[ i ]._webarPlaying = false;
					data.targets[ i ].forEach( function( target, index ) {
						if ( !data.options[ i ][ index ].tracking ) {
							data.options[ i ][ index ].scale = target.three( 0 ).scale.clone();
							target.three( 0 ).scale.set( 0, 0, 0 );
							target.three( 0 ).updateMatrix();
						}
					});

				}.bind( [ i ] ), 300 );

				data.targets[ i ] && data.targets[ i ].forEach( function( target, index ) {
					if ( !target.scaleX() ) {
						target.three( 0 ).scale.copy( data.options[ i ][ index ].scale );
						target.three( 0 ).updateMatrix();
					}
				});
		...
		data.targets.forEach( function( targets, i ) {
			data.frames[ i ].parent.updateMatrixWorld();
			data.frames[ i ]._webarPlaying && data.targets[ i ].forEach( function( target, index ) {
				target.three( 0 ).matrixWorld.multiplyMatrices( data.frames[ i ].children[ 0 ].matrixWorld, target.three( 0 ).matrix );
				target.three( 0 ).matrixWorldNeedsUpdate = false;
			});
		});
...
jThree.fn.WebAR = function( camera, glCanvas, option, beforeUpdate ) {
	if ( typeof option === "number" ) {
		var markerId = option;
		option = {};
	} else {
		var markerId = option.marker;
	...
	var flag = false;
	var that = this;
	option.scale = this.three( 0 ).scale.clone();
	this.three( 0 ).scale.set( 0, 0, 0 );
	this.three( 0 ).updateMatrix();
	this.three( 0 ).matrixAutoUpdate = false;

	dataArr.forEach( function( d ) {
		if ( d.glCanvas === glCanvas ) {
			d.options[ markerId ] = d.options[ markerId ] || [];
			d.options[ markerId ].push( option );

			d.frames[ markerId ] = d.frames[ markerId ] || j3( '<obj><obj><obj style="rotateX: 1.57;"/></obj></obj>' ).appendTo( camera ).three( 0 ).children[ 0 ];
			d.frames[ markerId ].useQuaternion = true;
			d.frames[ markerId ]._webarPastData = [];
			d.frames[ markerId ]._webarClearTimeout = null;
			d.frames[ markerId ]._webarPlaying = false;

			d.targets[ markerId ] = d.targets[ markerId ] || [];
			d.targets[ markerId ].push( that );
			flag = true;
		}
	});
	...
	var data = {
		...
		detector: new FLARMultiIdMarkerDetector( new FLARParam( canvas.width, canvas.height ), 10),
		beforeUpdate: beforeUpdate,
		pastResults: []
	};

	data.frames[ markerId ] = data.frames[ markerId ] || j3( '<obj><obj><obj style="rotateX: 1.57;"/></obj></obj>' ).appendTo( camera ).three( 0 ).children[ 0 ];
	data.targets[ markerId ] = [ this ];

	data.options[ markerId ] = [ option ];

	data.frames[ markerId ].useQuaternion = true;
	data.frames[ markerId ]._webarPastData = [];
	data.frames[ markerId ]._webarClearTimeout = null;
	data.frames[ markerId ]._webarPlaying = false;

	data.detector.setContinueMode(true);
...
jQuery.fn.getWebCam = function( idx ) {
	video = this[ 0 ];
	index = idx;
	init();
	return this;
	...
	window.MediaStreamTrack && MediaStreamTrack.getSources( function ( media_sources ) {
		...
		navigator.getUserMedia(
			...
			{ video: {
				...
				optional: [ { sourceId: camID[ index !== undefined ? index : camID.length - 1 ] } ]
...
window.videoToCanvas = function( video ) {
	...
	obj.update = function() {
		this.ctx.drawImage( this.video, ( this.W - this.w ) * .5, ( this.H - this.h ) * .5 );
	}.bind( obj );
